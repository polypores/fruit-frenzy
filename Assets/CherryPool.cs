...
public class CherryPool : MonoBehaviour
{
    ...
    GameObject Create(){ var go = Instantiate(cherryPrefab); go.SetActive(false); pool.Enqueue(go); return go; }
    ...
    public void ReturnCherry(GameObject go){ go.SetActive(false); pool.Enqueue(go); }
}
